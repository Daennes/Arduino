...
void loop() {
  ...
  if (str != "") {
    if (str == "1") {
      ...
      myTablo = makeTest();
      outputTablo(myTablo, "loop");
      myView = makeView(myTablo, cursorX, cursorY); 
	  outputView(myView, "main");
	  //output(myView);

//      outputView(myView, "loop");   
    } else {
...
//    width = getWidth(str);
//    Serial.print("Dim 1 : ");
//    Serial.println(tablohigh);
//    Serial.print("Dim 2 : ");
//    Serial.println(tablowidth);

  
    for (int i = 0 ; i < tabloHigh ; i++) {
      for ( int j = 0 ; j < tabloWidth ; j++) {
        tablo[tabloHigh][tabloWidth] = true;
      }
      Serial.println();
    }
    for (int i = 0 ; i < tabloHigh ; i++) {
      for ( int j = 0 ; j < tabloWidth ; j++) {
        Serial.print(tablo[i][j]);
      }
      Serial.println();
    }
    Serial.println();
